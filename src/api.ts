...
export async function api<T>(
  ...
  try {
    r = await fetch(`/api${path}`, {
      ...
      signal: AbortSignal.timeout(path === "/agent" || path.endsWith("/turns") ? 65000 : 30000),
    });
  } catch {
    throw new ApiError(
      "Unable to connect. Check your connection or open an offline pack.",
      0,
      "OFFLINE",
    );
  }
  const result = await r.json().catch(() => ({ error: "Invalid server response." }));
  if (!r.ok) throw new ApiError(result.error ?? "Request failed.", r.status, result.code);
  return result as T;
}
