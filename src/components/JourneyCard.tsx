    ...
    >
      <div className="journey-card-top">
        ...
        <span className="card-price">
          {money(j.price.totalCents)}
          <small>{j.travelers > 1 ? `${j.travelers} travelers total` : "total estimate"}</small>
        </span>
      </div>
      <div className="times">
        <strong>{time(j.departure, j.timezone)}</strong>
        <div className="duration-line">
          <small>{duration(j.durationMinutes)}</small>
          <span />
        </div>
        ...
        {j.legs
          ...
          .map((l, i) => (
            <span key={l.id}>
              ...
              <i className={`mode-pill mode-${l.mode}`}>
                <Icon name={modeIcon(l.mode)} size={15} />
                {l.operator}
              </i>
            </span>
          ))}
      ...
      <div className="journey-card-foot">
        ...
        <span className={`selection-indicator ${selected ? "checked" : ""}`}>
