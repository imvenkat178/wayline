...
export default function Inbox() {
  const { setActive, journeys, navigate, notify } = useApp();
  ...
  useEffect(() => {
    void run(load);
    const id = setInterval(() => void load().catch(() => {}), 30000);
  ...
  return (
    <>
              ...
              <div className="button-row">
                {a.journeyId && (
                  <Button
                    kind="small"
                    onClick={() => {
                      setActive(journeys.find((j) => j.id === a.journeyId) ?? null);
                      navigate("journey");
                    }}
                  >
                    Open journey
      ...
      <Notice>
        Monitoring runs while the Wayline server is running. Browser alerts require permission and
        an open app. Background push, SMS and email delivery are not connected.
      </Notice>
    </>
  );
}
