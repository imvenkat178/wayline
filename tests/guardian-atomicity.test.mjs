...
test("guardian: an alert and its push-delivery job commit atomically -- a failure enqueueing the job rolls back the alert too (R06)", () => {
  ...
  const now = Date.now();
  const user = store.createGuest();
  // R07: a pass-renewal reminder is info-severity, so it needs notifyInfo, and quiet hours
  // (evaluated against the real current time, since this test doesn't mock `now`) must be
  // disabled deterministically -- quietStart===quietEnd never matches, regardless of what time
  // this test happens to run -- or this test's push-job path wouldn't even attempt to enqueue.
...
test("reconcileOrphanedAlerts: an already-consistent alert (job already exists) is left untouched (R06)", () => {
  ...
  const now = Date.now();
  const user = store.createGuest();
  // R07: see the identical note on the atomicity test above -- notifyInfo and quiet hours both
  // have to be arranged so the initial guardian sweep actually queues the job this test then
  // checks is left alone.
  store.updateUser(user.id, {
    preferences: { notifyInfo: true, quietStart: "00:00", quietEnd: "00:00" },
  });
  pushSubscription(store, user.id);
