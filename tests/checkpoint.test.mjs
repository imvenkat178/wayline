...
test("stale GPS loses its live label without asserting cancellation", () => {
  const now = Date.now();
  const signal = freshness(
    { source: "live-gps", observedAt: new Date(now - 120000).toISOString() },
    now,
  );
...
test("booking needs a provider ticket and state changes must follow the graph", () => {
  assert.throws(() => transition({ state: "PLANNED", bookingConfirmed: false }, "BOOKED"), {
    code: "PROVIDER_REQUIRED",
  });
  assert.throws(() => transition({ state: "ARRIVED" }, "IN_TRANSIT"), {
...
test("record isolation, optimistic concurrency, share revocation and history deletion", (t) => {
  const store = temporaryStore(t),
    a = store.createGuest(),
    b = store.createGuest();
  const j = store.put(a.id, "journey", {
    from: "A",
    to: "B",
    legs: [],
    state: "PLANNED",
  ...
  assert.throws(() => store.put(a.id, "journey", j, { id: j.id, expectedVersion: j.version }), {
  ...
  for (const kind of ["search", "recovery", "idempotency"])
    store.put(a.id, kind, { journeyId: j.id });
...
test("HTTP session, CSRF, registration preferences and role boundaries", async (t) => {
  ...
  try {
    ...
    assert.equal(
      (await fetch(base + "/api/auth/register", { method: "POST", headers, body: "{}" })).status,
      403,
    );
    const registration = await fetch(base + "/api/auth/register", {
      method: "POST",
      headers: { ...headers, "x-csrf-token": boot.csrf },
      body: JSON.stringify({
        name: "Test",
    ...
    assert.equal(
      (await fetch(base + "/api/operator", { headers: { cookie: nextCookie } })).status,
      403,
    );
