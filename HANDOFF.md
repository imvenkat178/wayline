...
| Uncommitted | None (ignored paths such as `data/`, `tmp/`, `models/`, `.env` and `.env.container` excluded). |
...
1. `G2.4` The containers have only a liveness health check; a readiness probe covering SQLite, migrations and the job queue is the next hosting step that needs no decision.
2. `G6.1` The Launch 1 scope decides which hardening work matters; ask the owner to approve it.
3. `G7.2` Browser end-to-end tests are the largest remaining CI gap now that continuous integration is complete, and need no decision.
