...
- **What happened:** Last, on 2026-09-14, recorded that CI run 34917182505 passed on the G2.4 commit `fec38c5` (719/719 tests). Before that, at the owner's request, closed G2.4: added `GET /api/health/live` and `GET /api/health/ready`. Readiness checks the database, schema migrations and a per-lane job worker heartbeat, and returns 503 with the failing checks. The container health check now uses readiness, and `tests/health-probes.test.mjs` covers each failure. Before that, at the owner's request, added goal G11 and implemented GTFS-Realtime decoding for vehicles, trip updates and alerts (opt-in with `MBTA_REALTIME_SOURCE=gtfs-rt`), OpenTripPlanner vehicle-position and alert updaters, a schedule freshness status, clustered vehicle map markers, optional vector map style settings, and `scripts/transit-refresh.mjs` for G2.9. All were verified in the running container stack; the refresh rebuilt the graph and moved the schedule end date to 2026-11-16. Before that, finished G7.1.3 and G2.2 at the owner's request. The CI branch handoff check passed on GitHub (run 34909759816), closing G7.1. After Docker Desktop was restarted, the container stack was built and run end to end: the app image built, routing data downloaded, the Boston graph built, both services reported healthy, a South Station to Harvard search returned 5 journeys, and a session survived both an app restart and a restart after SIGKILL. That closed G2.2. Before that, PR #2 was merged into `main` as merge commit `2484230` after CI passed (run 34880174148 on its head `70e68a0`, then run 34880513668 on `main`, 704/704 tests), and the merged branch `chore/agent-memory-kit` was deleted locally and on GitHub. Before that, PR 1 was closed without merging, with its three plans kept in `docs/history/` (commit `917cb7d`), and PR #2 was opened from `chore/agent-memory-kit` into `main` (https://github.com/imvenkat178/wayline/pull/2). Its first CI run (34879457353) failed in `npm test` because 15 flight workspace tests asserted a Windows-only path separator; the test was fixed and pushed. Before that, a push of `chore/agent-memory-kit` was first rejected for a missing `workflow` scope; after the owner granted it, the branch was pushed using the GitHub CLI credential. Open PR 1 was also compared with the branch for G1.3 (conflicts in `README.md` and `docs/FEATURE_STATUS.md`). Earlier on 2026-09-14 added goal G10 (Documentation, 11 sub-goals) to `ROADMAP.md` and delivered its first references: `docs/README.md` (index), `docs/reference/API.md` (every endpoint), `docs/reference/CONFIGURATION.md` (all 59 environment variables), and `tests/docs-reference.test.mjs`, which fails when a server API path or environment variable is undocumented. Fixed out-of-date statements in `START_HERE.md` and `README.md`, completed `.env.example`, and added the references to the `AGENTS.md` documentation map. Then, at the owner's request, committed all remaining work, the local-Llama product release and this documentation, as one commit on `chore/agent-memory-kit` (G1.2, G10.1).
...
- **State left behind:** All work is on `main` through merge commit `2484230` (PR #2): `6b97a3d` (memory kit), `6529424` (local-Llama release and documentation), `917cb7d` (PR 1 plans kept as history), `c4fc03d` (Linux CI test fix) and `70e68a0` (CI record). That record was merged through PR #3 as `c542dff`, and both `chore/agent-memory-kit` and `chore/record-pr2-merge` are deleted. Later changes to the roadmap page, its generator and `AGENTS.md` (the "Working on now" panel and the artifact republish step) were committed directly on `main` at the owner's request and pushed. The G7.1.3 and G2.2 work was committed directly on `main` at the owner's request and pushed, and the verification results are recorded in a follow-up commit on `main`. The container stack was left running on this laptop; `docker compose stop` stops it and the named volumes keep its data. The G11 and G2.9 changes were committed directly on `main` at the owner's request and pushed; the running app container was rebuilt with them and uses the default V3 real-time source. The G2.4 changes were committed directly on `main` at the owner's request and pushed; the running app container was rebuilt with them.
- **Resume by:**
...
| Item | Current value |
| --- | --- |
| Branch | `main`, pushed to `origin/main` with a direct commit after merge commit `c542dff` (PR #3) (https://github.com/imvenkat178/wayline, public). `chore/agent-memory-kit` and `chore/record-pr2-merge` were deleted after their merges |
| Last commit | A direct commit on `main` (2026-09-14) adding GTFS-Realtime ingestion, schedule freshness, map clustering and the graph refresh script, after `31d30bf` (verified container stack record), after `b4ac28e` (CI branch check and container stack), after `e2b086f` (the "Working on now" roadmap panel and artifact republish step), after `c542dff` 2026-09-14 "Merge pull request #3 from imvenkat178/chore/record-pr2-merge" (handoff record `f7f44d2`), after `2484230` "Merge pull request #2 from imvenkat178/chore/agent-memory-kit", containing `70e68a0` (CI record), `c4fc03d` (CI test fix), `917cb7d` (PR 1 history), `6529424` (product release and documentation), `6b97a3d` (memory kit, 2026-09-13) and `0f02d7a` 2026-09-11 "Boston core launch: real transit planning, AI trip agent, recovery, PDFs, encrypted offline/backups" |
| Uncommitted | None; the G2.4 readiness work is in a direct commit on `main` (ignored paths such as `data/`, `tmp/`, `models/`, `.env` and `.env.container` excluded). |
...
- **CI (G7.1):** typecheck, lint, tests, build, both memory validators and a branch handoff check on pushes and pull requests to main; first full pass with the branch check in run 34909759816.
- **Health probes (G2.4):** `/api/health/live` answers while the process runs; `/api/health/ready` returns 503 with per-check details when SQLite, the schema migrations or a background job lane is unhealthy. The container health check uses readiness.
- **Containers (G2.2):** `Dockerfile` and `docker-compose.yml` build a 191 MB non-root app image and run it with OpenTripPlanner 2.10 and a saved Boston graph. Verified end to end on 2026-09-14 on Docker Desktop: health checks, a South Station to Harvard search, session persistence across `docker compose restart app`, and restart after SIGKILL. See README "Run with containers".
- **Transit data, routing and maps (G11.1 to G11.4, G11.6):** GTFS-Realtime VehiclePositions, TripUpdates and ServiceAlerts decode into the app's vehicle, prediction and alert shapes (`server/adapters/gtfsRealtime.mjs`, opt-in with `MBTA_REALTIME_SOURCE=gtfs-rt`, verified in the container stack). OpenTripPlanner runs trip update, vehicle position and alert updaters. The connection panel reports the routing graph's schedule end date. The map clusters vehicles, and OpenTripPlanner routes walking plus transit.
...
1. `G3.5` IP rate limits break behind any reverse proxy; this is a code-only P0 fix and the next hosting step that needs no decision.
2. `G6.1` The Launch 1 scope decides which hardening work matters; ask the owner to approve it.
3. `G2.5` Structured request logs and metrics build on the new health probes and need no decision.
4. `G7.2` Browser end-to-end tests are the largest remaining CI gap now that continuous integration is complete, and need no decision.
...
- The container stack has only run on this laptop's Docker Desktop, with a locally generated key in the git-ignored `.env.container`. The backup recovery key sits in the same data volume as the database (G2.7, G2.8). After each app start, the service connections panel shows sources as "not checked" until the first periodic or manual connection check runs, even while searches already work. No deployment pipeline, request logging or error tracking exists (G2.3, G2.5, G2.6). Readiness reports a job lane stale after three poll intervals (at least 30s) without a drain, or when one job runs longer than 10 minutes, so a slow backup or model call can briefly mark the container unhealthy.
...
- Docker Desktop 4.88.1 on this machine quits with "Windows Subsystem for Linux (WSL) is not installed" when it starts before WSL is running: its backend runs `c:\windows\system32\wsl.exe --version`, which failed at 2026-09-14T23:39Z. After any `wsl.exe` command had started WSL, a restart brought the engine up in about one minute. The "Windows Subsystem for Linux" optional feature is disabled (WSL 2.7.14 comes from the Store); enabling it, which needs admin rights and a reboot, may remove the race.
- MBTA real-time data still comes from the V3 JSON API by default. The GTFS-Realtime path was verified once in the container stack on 2026-09-14 and has not run for a longer period. A cancelled trip whose TripUpdate lists no stops is not reported as a cancelled prediction (G11.1).
- The local pilot's `.runtime/boston/router-config.json` keeps only the trip update updater until `npm run transit:setup` copies the new `infra/otp` configuration; the container stack already runs all three updaters.
- The clustered vehicle map and the vector style settings have not been checked visually in a browser, and no vector style provider has been chosen (G11.4, G11.5, G2.11).
...
| 2026-09-14 | Keep the MBTA V3 JSON API as the default real-time source; GTFS-Realtime is opt-in with `MBTA_REALTIME_SOURCE=gtfs-rt` | The V3 path has been validated over time; the GTFS-Realtime path makes other agencies possible and so far was verified once |
| 2026-09-14 | Keep OpenTripPlanner as the routing engine and Valhalla only for street routes, instead of adding OSRM or GraphHopper | OpenTripPlanner already routes walking plus scheduled GTFS transit with transfers; those engines do not route scheduled transit |
| 2026-09-14 | The container stack reads `.env.container`, not `.env` | Setting `DATA_ENCRYPTION_KEY` in `.env` would also change the key `npm start` uses and make the existing local database unreadable |
| 2026-09-14 | `infra/otp` holds the only copy of the OTP configuration | `scripts/setup-transit.mjs` and `scripts/otp-data.mjs` both copy it, so the local pilot and the containers build the same graph; its values match what the pilot already used |
| 2026-09-14 | Commit the roadmap "Working on now" panel and the artifact republish step directly on `main` | The owner asked to commit and push on `main` instead of using a pull request |
...
### 2026-09-14 | Claude Code (Claude Opus 5) | CI passed on the health probe commit
- **Goal:** Record the CI result for the G2.4 commit, and commit and push it as the owner asked.
- **Changed:** Updated sections 1 and 2 of this file and regenerated `docs/roadmap/index.html`. No code changed.
- **Verified:** `gh run view 34917182505` reported status completed, conclusion success, with checkout, `npm ci`, `npx tsc -b`, `npm run lint`, `npm test`, `npm run build`, `roadmap.py check`, `check_handoff.py --strict` and "Commits that change code also update HANDOFF.md" all successful. The run log showed 719 tests, 719 passed, 0 failed.
- **Not done / left broken:** The CI run for the commit carrying this entry is not recorded here.
- **Next agent should:** Take G3.5 (proxy-aware rate limiting), and ask the owner the pending P0 decisions G6.1, G2.1 and G2.10.

### 2026-09-14 | Claude Code (Claude Opus 5) | Separate liveness and readiness probes
- **Goal:** Implement G2.4, as the owner asked: separate liveness and readiness probes, with readiness failing when SQLite or the job workers are unavailable.
- **Changed:**
  - **Workers:** `server/jobWorkers.mjs` records when each lane last drained and whether it is busy, and exposes `status(now)`.
  - **Probes:** `server/server.mjs` adds `/api/health/live` and `/api/health/ready`, which checks `SELECT 1`, `schema_version` and migrated columns, and the job worker heartbeat; `/api/health` is unchanged.
  - **Container and docs:** the `Dockerfile` health check calls readiness; `docs/reference/API.md` and `README.md` describe both probes.
  - **Tests:** added `tests/health-probes.test.mjs` (5 tests).
- **Verified:**
  - **Tests:** `node --test tests/health-probes.test.mjs tests/docs-reference.test.mjs` 7/7 passed. `npm test` under PowerShell ran 719 tests with 717 passing, the 2 failures being the memory validators while the roadmap summary was stale. After `roadmap.py write`, `roadmap.py check` and `check_handoff.py --strict` passed, and a second `npm test` passed 719/719.
  - **Lint:** `npm run lint` 0 errors, 15 warnings (all pre-existing).
  - **Container:** `docker compose up -d --build app` exited 0. The recreated container reported Docker health `healthy` with the health check calling `/api/health/ready`. `/api/health/live` returned 200, and `/api/health/ready` returned 200 with database ok, migrations version 1 with nothing missing, and all four job lanes ok.
- **Not done / left broken:** At the owner's request this work was committed directly on `main` and pushed; the CI run for that push is not recorded here. `npx tsc -b` was not run because no TypeScript changed.
- **Next agent should:** Record the CI result for the G2.4 commit, then take G3.5 (proxy-aware rate limiting) and ask the pending P0 decisions.

### 2026-09-14 | Claude Code (Claude Opus 5) | GTFS-Realtime ingestion, schedule freshness, map clustering and graph refresh
- **Goal:** Implement the transit data, map and routing requirements the owner listed: GTFS and GTFS-Realtime ingestion, real-time updaters, schedule freshness, scheduled refresh, map clustering and vector tiles, and multimodal routing.
- **Changed:**
  - **GTFS-Realtime:** added `server/adapters/gtfsRealtime.mjs`. `server/adapters/providers.mjs` and `server/travel/providers.mjs` switch vehicles, alerts and predictions on `MBTA_REALTIME_SOURCE`, and the existing `/api/gtfs-rt` decoder now uses the shared module.
  - **Schedule freshness:** added a `schedule` travel tool (`otpServiceWindow` in `server/travel/routing.mjs`) and a "Boston schedule data" connection status, included in the connection check.
  - **OpenTripPlanner:** added vehicle-position and alert updaters to `infra/otp/router-config.json`.
  - **Maps:** `src/components/JourneyMap.tsx` now clusters vehicles and accepts `MAP_STYLE_URL`, which `server/securityPolicy.mjs` and bootstrap support along with `MAP_TILE_ORIGINS`.
  - **Refresh:** added `scripts/transit-refresh.mjs`.
  - **Tests:** added `tests/gtfs-realtime.test.mjs`, and updated the probe call count in `tests/travel-connection.test.mjs`.
  - **Docs and roadmap:** updated the configuration and API references, README and both env examples. Added goal G11 and advanced G2.9.
- **Verified:**
  - **Live feeds:** MBTA VehiclePositions.pb, TripUpdates.pb and Alerts.pb decoded with 527, 1169 and 122 entities.
  - **Static checks:** `node --check`, `npx eslint` and `npx tsc -b` passed, and a Vite build into `tmp/` transformed 323 modules.
  - **Tests:** `npm test` under PowerShell ran 714 tests with 712 passing. The 2 failures were the memory validators, failing because the roadmap summary was stale.
  - **Real-time updaters:** after copying the config into the container volume, OTP logged "OTP UPDATERS INITIALIZED (3 updaters)".
  - **GTFS-Realtime in the container:** with `MBTA_REALTIME_SOURCE=gtfs-rt` temporarily in `.env.container`, the rebuilt app served 500 GTFS-realtime vehicles and 121 alerts. After a connection check all 8 sources reported connected, including "Boston schedule data" valid until 2026-11-15T05:00:00Z with 61 days remaining. A search returned 5 walk-metro-walk journeys. The setting was then removed and the app restarted on the default source.
  - **Refresh script:** `node scripts/transit-refresh.mjs` exited 0 in 182s, kept `graph.obj.previous`, and reported schedules to 2026-11-16T05:00Z. OTP then initialized 3 updaters, and a search returned 5 journeys.
- **Not done / left broken:**
  - **Committed after this entry:** at the owner's request the changes were committed directly on `main` and pushed; the CI run for that push is not recorded here.
  - **Unexercised paths:** the refresh script's restore branch and the GTFS-Realtime path over a longer period.
  - **Browser checks:** the clustered map and vector style settings were not checked in a browser, and no vector style provider is chosen (G2.11).
  - **Hosted scheduling:** the hosted refresh schedule waits on G2.1.
  - **Cancelled trips:** a cancelled trip that lists no stops is not reported as a cancelled prediction.
- **Next agent should:** Record the CI result for the G11 commit, then take G2.4 (readiness probe) or G3.5 (proxy-aware rate limiting), and ask the owner the pending P0 decisions.

### 2026-09-14 | Claude Code (Claude Opus 5) | CI branch check and container stack verified
- **Goal:** Finish G7.1.3 and G2.2: confirm the branch check on GitHub, and build and run the container stack end to end.
- **Changed:** Checked G7.1.3 and closed G7.1; checked G2.2.4, strengthened the G2.2.5 evidence and closed G2.2. Updated sections 1 to 4 and 6 of this file, including a known limit about Docker Desktop starting before WSL. Created a git-ignored `.env.container` with a generated key. Left the stack running.
- **Verified:** CI run 34909759816 on `b4ac28e` succeeded, including the step "Commits that change code also update HANDOFF.md", which compared with `e2b086f`. Docker Desktop's first start logged "wsl is not installed" and quit; after WSL was running, a restart brought engine 29.7.2 up in about one minute. `docker compose build app` took 66s (191 MB image, user `node`, health check present). An image-only smoke test on port 4175 returned `/api/health` 200 and Docker health `healthy`, kept the session user after `docker restart`, and restarted after `docker kill --signal KILL` left `.server-pid` holding 1. The test container and volume were then removed. `docker compose run --rm otp-data` took 179s (mbta.gtfs.zip 25 MB, massachusetts.osm.pbf 310 MB). `docker compose run --rm otp-build` took 242s (OTP 2.10.0, graph.obj 360.6 MB). `docker compose up -d` reported app and otp healthy. The check script got `/api/health` 200, a bootstrap session with pilot true, and `POST /api/search` South Station to Harvard HTTP 200 with 5 walk-metro-walk journeys. After `docker compose restart app`, the saved session resolved to the same user. The travel panel showed "Boston routing" unavailable in the first process and "not checked" after the restart; after `POST /api/travel/check`, all seven sources reported connected. On the Windows host, the check script's own Node process hit a libuv assertion while exiting after printing its passing result.
- **Not done / left broken:** Not run from a separate fresh clone: the build used the committed working tree plus `.env.container`. The pull-request branch of the CI branch check has not run yet. The stack is still running and holds several GB of memory.
- **Next agent should:** Take G2.4 (readiness probe) or G3.5 (proxy-aware rate limiting), and ask the owner the pending P0 decisions.

### 2026-09-14 | Claude Code (Claude Opus 5) | Branch handoff check in CI and a container stack
- **Goal:** Work on G7.1.3 (branch handoff check in CI) and G2.2 (container image and compose stack), as the owner asked.
- **Changed:** `.github/workflows/ci.yml` checks out full history and, as its last step, runs `check_handoff.py --base` against `origin/<base branch>` on pull requests and against the previous commit on pushes. Added `Dockerfile`, `.dockerignore`, `docker-compose.yml`, `.env.container.example` and `scripts/otp-data.mjs`. `infra/otp` now holds the validated OTP configuration and `scripts/setup-transit.mjs` copies it. `server/server.mjs` ignores a stale `.server-pid` holding its own PID. Documented the stack in README.md and the new variables in `docs/reference/CONFIGURATION.md`, added the container files to `.agent-memory.json`, and ignored `.env.container`.
- **Verified:** `docker compose config` exited 0 using a temporary `.env.container` that was then removed; it lists `app` and `otp`, plus `otp-data` and `otp-build` under the `setup` profile. PyYAML parsed `ci.yml` (10 steps, `fetch-depth: 0`, branch check last) and `docker-compose.yml`. `node --check` and `npx eslint` passed on `scripts/otp-data.mjs`, `scripts/setup-transit.mjs` and `server/server.mjs`. Under PowerShell, `tests/docs-reference.test.mjs` and `tests/core-launch.test.mjs` passed; the two memory validator tests failed only because the roadmap summary was stale at that moment and are re-run below. `infra/otp` configs were JSON-identical to `.runtime/boston`, and `OTP_DATA_DIR=.runtime/boston node scripts/otp-data.mjs` reused both downloads and exited 0. Docker Hub showed `opentripplanner/opentripplanner:2.10.0` for amd64 and arm64 (entrypoint `/docker-entrypoint.sh`, volume `/var/opentripplanner`) and `node:24-bookworm-slim`. `docker info` could not reach the engine. After this entry, `roadmap.py write`, `roadmap.py check`, `check_handoff.py --strict` and the memory tests run, and the roadmap artifact is republished.
- **Not done / left broken:** The image had never been built and the stack had never run when this entry was written (G2.2.4). The CI branch check had not run on GitHub, so G7.1.3 stays open until a run is recorded. At the owner's request this work was committed directly on `main` and pushed.
- **Next agent should:** With the owner's approval, commit and push so CI exercises the branch check, then start Docker Desktop and complete G2.2.4.

### 2026-09-14 | Claude Code (Claude Opus 5) | Roadmap page shows current work and is republished every session
- **Goal:** Keep the owner's roadmap artifact current with the shared memory, and show which goals are being worked on.
- **Changed:** Added a "Working on now" panel to `docs/roadmap/index.html` (section 4 focus in order with titles and statuses, the newest session-log entry, and every `doing` sub-goal) plus an "In progress" status filter. `scripts/render-memory-pages.mjs` now parses HANDOFF.md sections 1, 4 and 8 into the page. Added step 8 to the `AGENTS.md` finish checklist: regenerate the page and republish it to https://claude.ai/code/artifact/6a5c3e70-d282-43b0-ab8e-00df1697dfc6. Corrected stale statements in sections 1 and 2 after PR #3 merged as `c542dff`.
- **Verified:** `npm run docs:memory` regenerated both pages; `python scripts/roadmap.py check` and `python scripts/check_handoff.py --strict` passed; `npx eslint scripts/render-memory-pages.mjs` passed. The injected data parsed from the page listed focus G7.1, G6.1, G2.2, G3.5, G2.7, the newest session entry, and doing sub-goals G7.1, G10.2 and G10.3; the page's inline script parsed without syntax errors. The page is regenerated again after this entry and republished to the artifact URL as the final step.
- **Not done / left broken:** At the owner's request these changes were committed directly on `main` and pushed; the CI run for that push is not recorded here. The page was not visually checked in a browser before publishing.
- **Next agent should:** Follow `AGENTS.md` step 8 at the end of every session, then take G7.1.3 or the next item from `python scripts/roadmap.py next`.
